/**
 *  1. create a project console.google.com 
 *  2. create firebase hosting just tick auth hosting 
 *  3. npm install firebase paste on the vs code and press Enter
 *  4. copy the firebase initialize code and paste the creating firebase-init.js file and  type bottom at the file export default app.
 * 5. npm install firebase on the vs code
 * 6. type const auth = getAuth(app) in the bottom, firebase initialize file and type top export {getAuth} from 'firebase/auth'; and bottom export default auth;
 * 7. firebase setting > authenticating > Enable Email auth options 
 * 8. npm install --save react-firebase-hooks
 * 9. useSignInWithEmailAndPassword from react firebase hooks
 * 10. create RequireAuth Component ==> Check user exist 
 * 11. in route wrap protect component by using require Auth component

 * 
 * 9. hosting steps- 
 *        1. npm install -g firebase-tools
 *        2. firebase login
 *        3. firebase init
 *        4. firebase deploy
 * 
 */